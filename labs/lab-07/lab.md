...
if __name__ == '__main__':
    G = words_graph()
    print("Loaded words_dat.txt containing {} five-letter English words.".format(nx.number_of_nodes(G)))
    print("Two words are connected if they differ in one letter.")
    print("Graph has %d nodes with %d edges"
          % (nx.number_of_nodes(G), nx.number_of_edges(G)))
    print("%d connected components" % nx.number_connected_components(G))
...
In order to allow for unordered implementation, we just have to edit `edit_distance_one`.

```python
 def edit_distance_one(word2):
    for word in permutations(word2): 
        word = ''.join(word)
        for i in range(len(word)):
            left, c, right = word[0:i], word[i], word[i + 1:]
            j = lookup[c]  # lowercase.index(c)
